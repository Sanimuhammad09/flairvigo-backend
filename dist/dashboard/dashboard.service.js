...
let DashboardService = class DashboardService {
    ...
    async getAnalytics() {
        ...
        const [currentMonthRevenue, lastMonthRevenue, currentMonthOrders, lastMonthOrders, currentMonthCustomers, lastMonthCustomers, topProducts, orderStatusBreakdown,] = await Promise.all([
            this.prisma.order.aggregate({
                where: {
                    status: { not: 'CANCELLED' },
        ...
        const topProductDetails = await Promise.all(topProducts.map(async (item) => {
